...
const ManagePage: React.FC = () => {
  ...
  /* ---------------- 从数据库获取数据的 Hooks ---------------- */
  const { data: isleaderData, refetch: refetchLeader } = graphql.useIsTeamLeaderSuspenseQuery({
    variables: {
      _id: userInfo?._id!,
  ...
  const { data: ismemberData, refetch: refetchMember,} = graphql.useIsTeamMemberSuspenseQuery({
    variables: {
      _id: userInfo?._id!,
      contest_id: Contest_id,
    },
  ...
  //根据team_id查询所有队员信息
  const { data: teamMemberData, refetch: refetchMemberInfo,} = graphql.useGetMemberInfoQuery({
    variables: {
      team_id: teamid!,
    },
  });
  ...
  //更新队伍信息
  const [UpdateTeam, { data: UpdateTeamData, error: UpdateTeamError },] = graphql.useUpdateTeamMutation();
  //删除队伍信息
  const [DeleteTeam, { error: DeleteTeamError }] = graphql.useDeleteTeamMutation();
  //删除所有队员
  const [DeleteAllTeamMember, { error: DeleteAllTeamMemberERROR }] =graphql.useDeleteAllTeamMemberMutation();
    ...
    <Result
      ...
      extra={
        <div style={{ display: 'flex', flexDirection: 'column' ,justifyContent: 'center', alignItems: 'center' }}>
          <Button type="primary" style={{ marginBottom: '20px', width: '180px', height: '40px' }}>
                ...
                <Form.Item label="队员">
                  <Suspense fallback={<Loading />}>
                    <Table
                      ...
                      dataSource={teamMemberData?.contest_team_member as GetMemberInfo_contest_team_member[]}
